...
class CBDNetChannelEstimator(nn.Module):
    """CBDNet-style noise-conditioned channel denoiser.

    Stage 1 estimates a token-wise noise map from the LS-like channel. Stage 2
    performs non-blind denoising conditioned on both the LS-like input and the
    estimated noise map. Optional RSS context can be injected through
    cross-attention, but the key difference from the previous noise-aware
    baseline is that the estimated noise map is an explicit input to the
    ...
    def __init__(
        ...
        self.noise_blocks = nn.ModuleList(
            [ConvTokenBlock(hidden_dim, dropout=dropout) for _ in range(max(1, depth))]
        )
        ...
        self.denoise_in = nn.Linear(4, hidden_dim)
        self.denoise_pos = nn.Parameter(torch.zeros(1, num_channel_tokens, hidden_dim))
        ...
        self.rss_norm = nn.LayerNorm(hidden_dim)
        ...
        self.global_refiner = nn.TransformerEncoder(encoder_layer, num_layers=1)
    ...
    def _reset_parameters(self):
        ...
        nn.init.normal_(self.denoise_pos, std=0.02)
    ...
    def forward(self, ls_tokens, rss, return_noise=False):
        ...
        for block in self.noise_blocks:
            noise_feat = block(noise_feat)
        noise_map = self.noise_head(noise_feat)

        denoise_input = torch.cat([ls_tokens, noise_map], dim=-1)
        denoise_feat = self.denoise_in(denoise_input) + self.denoise_pos
        for block in self.denoise_blocks:
            denoise_feat = block(denoise_feat)
        ...
        if self.use_rss_context:
            ...
            denoise_feat = self.rss_norm(denoise_feat + attended)

        denoise_feat = self.global_refiner(denoise_feat)
        residual = self.residual_head(denoise_feat)
        gate = self.noise_gate(noise_feat.mean(dim=1)).unsqueeze(1)
        pred = ls_tokens + gate * residual

        if return_noise:
            return pred, noise_map
        return pred
...
def run_epoch(model, loader, optimizer, device, train, noise_weight):
    ...
    for ls_tokens, rss, target_tokens in loader:
        ...
        with torch.set_grad_enabled(train):
            pred, noise_map = model(ls_tokens, rss, return_noise=True)
...
def main():
    parser = argparse.ArgumentParser(description="Train a DeepMIMO CBDNet-style channel baseline.")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Minimum Val NMSE improvement required to reset early stopping.",
    )
    ...
    suffix = f"cbdnet_{args.mode}_nw{str(args.noise_weight).replace('.', 'p')}"
    ...
    print(
        json.dumps(
            {
                "model": "cbdnet_channel_estimator",
                "mode": args.mode,
    ...
    for epoch in range(args.epochs):
        ...
        history.append(
            {
                ...
                "train_noise": train_metrics["noise"],
                "val_nmse": val_nmse,
            }
        )
        ...
        if val_nmse < best_val - args.min_delta:
            ...
            print(f"Saved best model at epoch {best_epoch} with Val NMSE={best_val:.6f}")
        else:
