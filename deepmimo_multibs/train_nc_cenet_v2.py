...
class NoiseConditionedCrossAttentionNet(nn.Module):
    """Cross-attention residual refinement with explicit noise conditioning.

    This version keeps the cross-attention baseline's main refinement path and
    inserts a learned token-wise noise map as an extra channel condition. It is
    meant to test whether noise conditioning improves the existing baseline,
    instead of replacing that baseline with a different denoising backbone.
    """
    ...
    def __init__(
        ...
        self.noise_encoder = nn.TransformerEncoder(noise_layer, num_layers=max(1, noise_depth))
        ...
        self.channel_proj = nn.Linear(4, hidden_dim)
        ...
        self.channel_refiner = nn.TransformerEncoder(refiner_layer, num_layers=depth)
    ...
    def forward(self, ls_tokens, rss, return_noise=False):
        ...
        conditioned = torch.cat([ls_tokens, noise_map], dim=-1)
        channel_tokens = self.channel_proj(conditioned) + self.channel_pos
        ...
        refined = self.cross_norm(channel_tokens + attended)
        refined = self.channel_refiner(refined)

        residual = self.residual_head(refined)
        gate = self.gate_head(refined).clamp(0.05, 0.95)
...
def main():
    parser = argparse.ArgumentParser(description="Train NC-CENet v2 on DeepMIMO channel data.")
    ...
    parser.add_argument("--noise-depth", type=int, default=1)
    ...
    model = NoiseConditionedCrossAttentionNet(
    ...
    suffix = f"nc_cenet_v2_{args.mode}_nw{str(args.noise_weight).replace('.', 'p')}"
    out_dir = Path(args.out_dir) if args.out_dir else data_dir / "runs" / f"{suffix}_ep{args.epochs}_es{args.patience}"
